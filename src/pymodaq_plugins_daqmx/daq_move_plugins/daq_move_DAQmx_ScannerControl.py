...
class DAQ_Move_DAQmx_ScannerControl(DAQ_Move_base):
    """Plugin to control piezo scanners with a NI card.
    ...
    def ini_stage(self, controller=None):
        ...
        # we need to close the clock once the move is done, the free
        # the counter ressource
    ...
    def prepare_voltage_list(self):
        ...
        if np.abs(self.target_position - self.current_position) <= self.step_size:
            self.voltage_list = np.array([self.target_position])/self.conv_factor
        # otherwise we do steps
        else:
            pos_list = np.arange(min(self.current_position, self.target_position),
                                 max(self.current_position, self.target_position)+self.step_size,
                                 self.step_size)
            # we need to start from the beginning
            if pos_list[0] != self.current_position:
                pos_list = pos_list[::-1]
            # convert to voltage
